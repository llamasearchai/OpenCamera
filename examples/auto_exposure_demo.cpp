...
public:
  ...
  void run() {
    ...
    while (running_) {
      ...
      if (std::chrono::duration_cast<std::chrono::seconds>(now - lastStatsTime)
              .count() >= 2) {
...
private:
  void createControlPanel() {
    ...
    cv::createTrackbar("Target Brightness", "Controls", nullptr, 100,
                       onTargetBrightnessChange, this);
    cv::createTrackbar("Convergence Speed", "Controls", nullptr, 100,
                       onConvergenceSpeedChange, this);
    cv::createTrackbar("Exposure Compensation", "Controls", nullptr, 200,
                       onExposureCompensationChange, this);
    cv::createTrackbar("Metering Mode", "Controls", nullptr, 4,
                       onMeteringModeChange, this);

    // Set initial values
    auto params = controller_->getParameters();
    cv::setTrackbarPos("Target Brightness", "Controls",
                       static_cast<int>(params.targetBrightness * 100));
    cv::setTrackbarPos("Convergence Speed", "Controls",
                       static_cast<int>(params.convergenceSpeed * 100));
    cv::setTrackbarPos(
        "Exposure Compensation", "Controls",
        static_cast<int>((params.exposureCompensation + 2.0f) * 50));
    cv::setTrackbarPos("Metering Mode", "Controls",
  ...
  void addOverlayInfo(cv::Mat &image, const CameraFrame &frame,
                      float exposure) {
    // Get statistics
    ...
    overlayText.push_back(
        fmt::format("Target: {:.2f}", params.targetBrightness));
    overlayText.push_back(
        fmt::format("Avg Brightness: {:.3f}", stats.averageBrightness));
    overlayText.push_back(
        fmt::format("Converged: {}", stats.isConverged ? "Yes" : "No"));
    ...
    if (params.enableSceneAnalysis) {
      ...
      overlayText.push_back(fmt::format(
          "Low Light: {}", sceneAnalysis.isLowLight ? "Yes" : "No"));
      overlayText.push_back(
          fmt::format("Backlit: {}", sceneAnalysis.isBacklit ? "Yes" : "No"));
      overlayText.push_back(
          fmt::format("Faces: {}", sceneAnalysis.faceRegions.size()));
    }

    // Draw overlay
    int y = 30;
    for (const auto &text : overlayText) {
      cv::putText(image, text, cv::Point(10, y), cv::FONT_HERSHEY_SIMPLEX, 0.6,
                  cv::Scalar(0, 255, 0), 2);
      y += 25;
    }
  ...
  void drawHistogram(cv::Mat &image) {
    ...
    cv::line(image, cv::Point(image.cols - histWidth - 10 + targetX, 10),
  ...
  void drawConvergenceIndicator(cv::Mat &image, bool converged) {
    cv::Scalar color =
        converged ? cv::Scalar(0, 255, 0) : cv::Scalar(0, 0, 255);
  ...
  void updateStatistics(int frameCount,
    ...
    spdlog::info("FPS: {:.1f}, Avg Exposure: {:.4f}, Avg Brightness: {:.3f}, "
                 "Converged: {}",
                 fps, stats.averageExposure, stats.averageBrightness,
                 stats.isConverged);
  }
  ...
  void handleKeyPress(int key) {
    ...
    case 'f':
      ...
      spdlog::info("Face detection: {}",
                   params.enableFaceDetection ? "ON" : "OFF");
      break;
    case 's':
      params.enableSceneAnalysis = !params.enableSceneAnalysis;
      spdlog::info("Scene analysis: {}",
                   params.enableSceneAnalysis ? "ON" : "OFF");
      break;
    case 'r':
      controller_->reset();
    ...
    case '=':
      params.exposureCompensation =
          std::min(2.0f, params.exposureCompensation + 0.1f);
      spdlog::info("Exposure compensation: {:.1f} EV",
                   params.exposureCompensation);
      break;
    case '-':
      params.exposureCompensation =
          std::max(-2.0f, params.exposureCompensation - 0.1f);
      spdlog::info("Exposure compensation: {:.1f} EV",
                   params.exposureCompensation);
      break;
    case 'h':
  ...
  std::string getModeString(AutoExposureController::Mode mode) {
    switch (mode) {
    case AutoExposureController::Mode::AVERAGE:
      return "Average";
    case AutoExposureController::Mode::CENTER_WEIGHTED:
      return "Center-weighted";
    case AutoExposureController::Mode::SPOT:
      return "Spot";
    case AutoExposureController::Mode::MULTI_ZONE:
      return "Multi-zone";
    case AutoExposureController::Mode::INTELLIGENT:
      return "Intelligent";
    default:
      return "Unknown";
    }
...
void printUsage(const char *programName) {
  std::cout << "OpenCam Auto Exposure Demo" << std::endl;
  std::cout << "Author: Nik Jois <nikjois@llamasearch.ai>" << std::endl;
  std::cout << std::endl;
  std::cout << "Usage: " << programName << " [OPTIONS] [CAMERA_ID]"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "  -q, --quiet    Enable quiet mode (minimal logging)"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "  " << programName << "              # Use default camera (0)"
            << std::endl;
  std::cout << "  " << programName << " 1            # Use camera device 1"
            << std::endl;
  std::cout << "  " << programName
            << " --debug 2    # Use camera 2 with debug logging" << std::endl;
  std::cout << std::endl;
...
void printVersion() {
  std::cout << "OpenCam Auto Exposure Demo v1.0.0" << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "-h" || arg == "--help") {
      ...
      // Non-option argument is camera ID
      cameraId = arg;
    } else {
