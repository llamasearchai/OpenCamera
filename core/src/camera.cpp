...
public:
  ...
  CameraState getState() const override { return state_; }

  CameraProperties getProperties() const override { return properties_; }
  ...
  bool setWhiteBalance(float redGain, float greenGain,
                       float blueGain) override {
    if (state_ == CameraState::DISCONNECTED || state_ == CameraState::ERROR) {
      return false;
    }
  ...
  std::shared_ptr<CameraFrame> getNextFrame() override {
    ...
    frame->timestamp =
        static_cast<int64_t>(1000.0 * capture_.get(cv::CAP_PROP_POS_MSEC));

    // Add metadata
    frame->metadata["exposure"] =
        static_cast<float>(capture_.get(cv::CAP_PROP_EXPOSURE));
    frame->metadata["brightness"] =
        static_cast<float>(capture_.get(cv::CAP_PROP_BRIGHTNESS));
...
CameraManager::getCamera(const std::string &deviceId) {
