...
public:
  ...
  struct Parameters {
    ...
    float exposureCompensation =
        0.0f; ///< Manual exposure compensation (-2.0 to +2.0 EV)
    int analysisDownscale =
        4; ///< Downscale factor for analysis (>=1). 1 = full res
  };
...
private:
  ...
  // Convergence tracking (guarded for thread safety)
  ...
  float computeBrightnessWithContext(const cv::Mat &gray,
  ...
  float computeIntelligentBrightness(const cv::Mat &gray,
                                     const SceneAnalysis &scene);
  float computeFacePriorityBrightness(const cv::Mat &gray,
                                      const std::vector<cv::Rect> &faces);
  float computeBacklitBrightness(const cv::Mat &gray);

  // Intelligent exposure computation
  float computeIntelligentExposure(float currentBrightness,
                                   const SceneAnalysis &scene);
  float getAdaptiveTarget(const SceneAnalysis &scene);
  float computeExposureCompensation(const SceneAnalysis &scene,
                                    float brightnessError);
  ...
  // Fast-path helpers for performance when ML is disabled
  float computeBrightnessFast(const cv::Mat &gray);
